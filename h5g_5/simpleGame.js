...
function Camera(scene) {
    ...
    this.checkFocusBounds = function() {
        ...
        if (Math.abs(this.focalPointY - centerY) >= this.waitY) { return false; } else { return true; }
    }
}
        ...
        for (i = 0; i < this.mapData.length; i++) { //for each row
            for (j = 0; j < this.mapData[i].length; j++) { //for each column of each row
                ...
                if (0 < drawX < this.camera.cWidth && 0 < drawY < this.camera.cHeight) { //don't draw any tiles that will not be in the camera's view
                    ...
                    if (this.tiles[i][j].animationPlaying) { this.drawTileAnimation(this.tiles[i][j], ctx); } else {
...
function Sprite(scene, imageFile, width, height) {
    ...
    */
    ...
    this.draw = function() {
            ...
            ctx.save();
            if (this.camera) { ctx.translate(this.x - this.camera.cameraOffsetX, this.y - this.camera.cameraOffsetY); } else { ctx.translate(this.x, this.y); }
